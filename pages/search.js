        ...
        <div className="flex w-full p-6 items-center">
          ...
          >
            <input
              ...
              className="flex flex-1 w-full focus:outline-none"
            />

            <XIcon
              className="cursor-pointer h-5 text-gray-500 transition duration-100 transform 
                        hover:scale-125 sm:m-3"
              onClick={() => (searchInputRef.current.value = "")}
            />
            <MicrophoneIcon
              className="h-5 text-blue-500 hover:scale-125 sm:m-2 hidden  sm:inline-flex border-l-2
...
export async function getServerSideProps(context) {
  ...
  const data = useDummyData
    ? Response
    : await fetch(
        `https://customsearch.googleapis.com/customsearch/v1?cx=${process.env.CONTEXT_KEY}&q=${context.query.term}&key=${process.env.API_KEY}&start=${startIndex}`
      ).then((response) => response.json());
